import { logObj } from "../compile_simple/utils.mjs";
...
const TextModes = {
  ...
  RCDATA: "RCDATA",
  // <style>、<xmp>、<iframe>、<noembed>、<noframes>、<noscript> 等标签
  RAWTEXT: "RAWTEXT",
  // <![CDATA[ 字符串
  CDATA: "CDATA",
};
    ...
    if (mode === TextModes.DATA || mode === TextModes.RCDATA) {
      ...
      if (mode === TextModes.DATA && source[0] === "<") {
        if (source[1] === "!") {
          ...
          // 状态机遭遇了闭合标签，此时应该抛出错误，因为它缺少与之对应的开始标签，例如<div><span></div></span>
...
function parseAttributes(context) {
  ...
  while (!context.source.startsWith(">") && !context.source.startsWith("/>")) {
    ...
    props.push({
      type: /(v-bind:)|(v-on:)|(@)|(:)/.test(name) ? "Directive" : "Attribute",
      name,
      value,
    });
  }
...
logObj(
  "ast",
  parse(
    // '<div class="container" :id="dynamicId" @click="handler" v-on:mousedown="onMouseDown" >Text&ltcc{{ bar }} baz</div>'
    "<div>foo {{bar}} baz</div>"
    // "<div><!-- comments --></div>"
  )
);
